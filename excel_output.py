...
class ExcelOutput:
    ...
    def create_excel_file(self, results_dir):
        ...
        headers = ["Subject", "EEG", "VF", "Label", "Avg Before", "Min Avg After", "%"]
    ...
    def build_event_dict(self, event_stream):
        ...
        for event in event_dict:
            event_end = round(event_dict[event].get("stop"), self.DIGITS_FIXED)
            next_start = round(event_dict[event].get("next_event_start"), self.DIGITS_FIXED)
    ...
    def compile_filename(self):
        ...
        return f"Data-{self.subject_id}-{self.eeg_part}-{date_str}-{time_str}"
